...
class BarrierTypeTable extends DataTableComponent
{
    protected $model = BarrierType::class;
    ...
    public function configure(): void
    {
        ...
        $this->setPrimaryKey('id')
            ->setTableRowUrl(function($row) {
                return route('barrier-type.edit', $row);
            });
    }
    ...
    {
        if (auth()->user()->can('BarrierType delete')) {
            $model = $this->getSelected();
            foreach ($model as $key => $value) {
                $model_one = BarrierType::find($value);
                $model_one?->delete();
            }
            $this->clearSelected();
        } else {
