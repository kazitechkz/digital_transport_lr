...
class LightingSupportTypeTable extends DataTableComponent
{
    protected $model = LightingSupportType::class;
    ...
    public function configure(): void
    {
        ...
        $this->setPrimaryKey('id')
            ->setTableRowUrl(function($row) {
                return route('lighting-support-type.edit', $row);
            });
    }
    ...
    {
        if (auth()->user()->can('LightingSupportType delete')) {
            $model = $this->getSelected();
            foreach ($model as $key => $value) {
                $model_one = LightingSupportType::find($value);
                $model_one?->delete();
            }
            $this->clearSelected();
        } else {
