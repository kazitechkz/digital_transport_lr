...
class TypeTrafficLightTable extends DataTableComponent
{
    protected $model = TypeTrafficLight::class;
    ...
    public function configure(): void
    {
        ...
        $this->setPrimaryKey('id')
            ->setTableRowUrl(function($row) {
                return route('type-traffic-light.edit', $row);
            });
    }
    ...
    {
        if (auth()->user()->can('TypeTrafficLight delete')) {
            $model = $this->getSelected();
            foreach ($model as $key => $value) {
                $model_one = TypeTrafficLight::find($value);
                $model_one?->delete();
            }
            $this->clearSelected();
        } else {
