...
class RoadMarkingMaterialTable extends DataTableComponent
{
    protected $model = RoadMarkingMaterial::class;
    ...
    public function configure(): void
    {
        ...
        $this->setPrimaryKey('id')
            ->setTableRowUrl(function($row) {
                return route('road-marking-material.edit', $row);
            });
    }
    ...
    {
        if (auth()->user()->can('RoadMarkingMaterial delete')) {
            $model = $this->getSelected();
            foreach ($model as $key => $value) {
                $model_one = RoadMarkingMaterial::find($value);
                $model_one?->delete();
            }
            $this->clearSelected();
        } else {
