...
class CommunicationTypeTable extends DataTableComponent
{
    ...
    public function configure(): void
    {
        ...
        $this->setPrimaryKey('id')
            ->setTableRowUrl(function($row) {
                return route('communication-type.edit', $row);
            });
    }
    ...
    {
        if (auth()->user()->can('CommunicationType delete')) {
            $model = $this->getSelected();
            foreach ($model as $key => $value) {
                $model_one = CommunicationType::find($value);
                $model_one?->delete();
            }
            $this->clearSelected();
        } else {
