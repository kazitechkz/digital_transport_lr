...
class FlooringConditionTable extends DataTableComponent
{
    protected $model = FlooringCondition::class;
    ...
    public function configure(): void
    {
        ...
        $this->setPrimaryKey('id')
            ->setTableRowUrl(function($row) {
                return route('flooring-condition.edit', $row);
            });
    }
    ...
    {
        if (auth()->user()->can('FlooringCondition delete')) {
            $model = $this->getSelected();
            foreach ($model as $key => $value) {
                $model_one = FlooringCondition::find($value);
                $model_one?->delete();
            }
            $this->clearSelected();
        } else {
